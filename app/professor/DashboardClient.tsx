...
import {
    ...
    AlertTriangle, MessageCircle, Trophy
} from "lucide-react";
...
// --- IMPORTAÇÃO DIRETA ---
import { reformularTribos, sincronizarTudo, zerarPlacarTribos, excluirAluno } from "./actions"; // <--- Importei excluirAluno e zerarPlacarTribos
...
export default function DashboardClient({
                                            ...
                                            onExcluirPresenca,
                                            onExcluirLicao,
                                            onExcluirAluno: propExcluirAluno, // Renomeamos a prop para usar a action direta
                                        }: DashboardProps) {
    ...
    const handleSincronizar = async () => {
        if(confirm("Isso vai recalcular o XP de todos os alunos.\nDeseja continuar?")) {
            setLoadingTribos(true);
            try {
                const res = await sincronizarTudo();
                alert(`Sincronização concluída!\n${res.count} alunos corrigidos.`);
    ...
    // 3. ZERAR PLACAR DAS TRIBOS (NOVO)
    const handleZerarPlacar = async () => {
        if(confirm("⚠️ ZERAR PLACAR?\n\nIsso vai colocar o XP de TODAS as tribos em 0.\nOs alunos continuam com o XP individual deles, mas o campeonato recomeça.\n\nTem certeza?")) {
            setLoadingTribos(true);
            try { await zerarPlacarTribos(); alert("Placar zerado! O campeonato recomeçou."); window.location.reload(); }
            catch (e) { alert("Erro."); }
            setLoadingTribos(false);
        }
    }

    // 4. EXCLUIR ALUNO (USANDO A ACTION NOVA QUE DESCONTA PONTOS)
    const handleExcluirAluno = async (id: string) => {
        if(confirm("Tem certeza? Isso apaga todo o histórico do aluno e REMOVE OS PONTOS DELE DA TRIBO!")) {
            await excluirAluno(id); // Chama a função nova do actions.ts
            alert("Aluno excluído e pontos removidos da tribo.");
            window.location.reload();
        }
    }
        ...
        <div className="flex min-h-screen bg-slate-950 text-white">
            ...
            <main className="flex-1 bg-slate-950 p-6 overflow-auto">
                ...
                <div className="md:hidden flex gap-2 mb-6 overflow-x-auto pb-2">
                    {[{id:"visao-geral",label:"Geral"},{id:"alunos",label:"Alunos"},{id:"presenca",label:"Presença"},{id:"licoes",label:"Lições"},{id:"config",label:"Config"}].map((item)=>(<button key={item.id} onClick={()=>setAbaAtiva(item.id)} className={`px-4 py-2 rounded-lg text-sm font-bold whitespace-nowrap transition-colors ${abaAtiva===item.id?"bg-violet-600 text-white border border-violet-500":"bg-slate-900 text-slate-400 border border-slate-800"}`}>{item.label}</button>))}
                </div>
                    ...
                    <div className="space-y-6 animate-in fade-in duration-500">
                        {alunosAusentes && alunosAusentes.length > 0 && (
                            <Card className="bg-red-950/20 border-red-900/50 mb-6">
                                <CardHeader className="pb-2"> <CardTitle className="text-red-400 flex items-center gap-2 text-lg"> <AlertTriangle /> Radar de Ovelhas (Ausentes +15 dias) </CardTitle> </CardHeader>
                                <CardContent>
                                    <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-3">
                                        {alunosAusentes.map(aluno => (
                                            <div key={aluno.id} className="bg-slate-900 border border-slate-800 p-3 rounded-lg flex justify-between items-center">
                                                <div> <p className="font-bold text-white">{aluno.name}</p> <p className="text-xs text-slate-400"> Última vez: {aluno.attendances && aluno.attendances.length > 0 ? new Date(aluno.attendances[0].date).toLocaleDateString('pt-BR') : "Nunca veio"} </p> </div>
                                                <Button size="icon" className="h-8 w-8 bg-green-600 hover:bg-green-700 rounded-full"> <MessageCircle size={16} /> </Button>
                                            </div>
                                        ))}
                                    </div>
                                </CardContent>
                ...
                {abaAtiva === "alunos" && (
                    <div className="space-y-6 animate-in fade-in duration-500">
                        ...
                        <div className="relative"> <Search className="absolute left-3 top-3 text-slate-500" size={18} /> <Input placeholder="Buscar por nome ou tribo..." className="pl-10 bg-slate-900 border-slate-800 text-white" value={termoBusca} onChange={(e) => setTermoBusca(e.target.value)} /> </div>
                        <Card className="bg-slate-900 border-slate-800">
                            <div className="max-h-[500px] overflow-auto">
                                <table className="w-full text-sm text-left">
                                    <thead className="text-slate-400 border-b border-slate-800 bg-slate-950/50 sticky top-0"> <tr> <th className="p-4">Nome</th> <th className="p-4">Tribo</th> <th className="p-4">Nível</th> <th className="p-4">XP</th> <th className="p-4 text-right">Ação</th> </tr> </thead>
                                    <tbody>
                                    {alunosFiltrados.map((aluno) => (
                                        <tr key={aluno.id} className="border-b border-slate-800 hover:bg-slate-800/50">
                                            ...
                                            <td className="p-4 text-right">
                                                {/* Botão de Excluir usando a nova função handleExcluirAluno */}
                ...
                {abaAtiva === "licoes" && (
                    <div className="space-y-6 animate-in fade-in duration-500">
                        <div className="flex justify-between items-center"> <h2 className="text-2xl font-bold text-white">Lições & Estudos</h2> <Link href="/professor/criar-licao"> <Button className="bg-violet-600 hover:bg-violet-700"> <PlusCircle size={18} className="mr-2" /> Nova Lição </Button> </Link> </div>
                            ...
                            {licoes.length === 0 ? <p className="text-slate-500 text-center py-10">Nenhuma lição criada ainda.</p> : licoes.map((licao) => (
                                <Card key={licao.id} className="bg-slate-900 border-slate-800 hover:border-violet-500/50 transition-colors">
                                    <CardContent className="p-6 flex justify-between items-center">
                                        ...
                                        <div className="flex gap-2 items-center"> <span className={licao.isPublished ? "px-2 py-1 rounded text-xs font-bold bg-green-500/10 text-green-400" : "px-2 py-1 rounded text-xs font-bold bg-yellow-500/10 text-yellow-400"}> {licao.isPublished ? "Publicado" : "Rascunho"} </span> <Link href={`/professor/editar-licao/${licao.id}`}> <Button size="sm" className="bg-slate-800 text-white border border-slate-700 hover:bg-slate-700"> <Pencil size={14} className="mr-2" /> Editar </Button> </Link> <Button variant="destructive" size="sm" className="bg-red-900/20 text-red-500 hover:bg-red-900/40 border border-red-900/50" onClick={() => onExcluirLicao(licao.id)}> <Trash2 size={14} /> </Button> </div>
                                    </CardContent>
                                </Card>
                            ))}
                        </div>
                ...
                {abaAtiva === "config" && (
                    <div className="space-y-6 animate-in fade-in duration-500">
                        ...
                        <div className="border border-red-800/30 bg-red-900/10 p-6 rounded-xl relative overflow-hidden">
                            ...
                            <p className="text-slate-400 text-sm mb-6 max-w-2xl leading-relaxed"> Esta ação irá <strong>excluir todas as tribos</strong> extras, mantendo apenas Judá 🦁 e Levi 🛡️. </p>
                            <Button onClick={handleReformular} disabled={loadingTribos} className="bg-red-600 hover:bg-red-700 text-white font-bold h-12 px-6 shadow-lg shadow-red-900/20"> {loadingTribos ? "Processando..." : "EXECUTAR REFORMULAÇÃO"} </Button>
                        </div>

                        <div className="border border-blue-800/30 bg-blue-900/10 p-6 rounded-xl mt-6 relative overflow-hidden">
                            <h3 className="text-blue-400 font-bold text-lg mb-2 flex items-center gap-2"> 🔄 Sincronização Geral </h3>
                            <p className="text-slate-400 text-sm mb-6 max-w-2xl"> Detectou algum erro? Saldo negativo? Dias seguidos não contando? <br/> Este botão <strong>recalcula tudo do zero</strong> para todos os alunos com base nas presenças confirmadas. </p>
                            <Button onClick={handleSincronizar} disabled={loadingTribos} className="bg-blue-600 hover:bg-blue-700 text-white font-bold h-12 px-6"> {loadingTribos ? "Calculando..." : "SINCRONIZAR E CORRIGIR DADOS"} </Button>
                        </div>

                        {/* --- NOVO BOTÃO: ZERAR PLACAR DAS TRIBOS --- */}
                        <div className="border border-yellow-800/30 bg-yellow-900/10 p-6 rounded-xl mt-6 relative overflow-hidden">
                            <h3 className="text-yellow-400 font-bold text-lg mb-2 flex items-center gap-2"> 🏆 Zerar Placar das Tribos </h3>
                            <p className="text-slate-400 text-sm mb-6 max-w-2xl"> Vai começar um novo trimestre? <br/> Use este botão para <strong>colocar o placar de todas as tribos em ZERO</strong>. (O XP individual dos alunos não muda, só o da tribo). </p>
                            <Button onClick={handleZerarPlacar} disabled={loadingTribos} className="bg-yellow-600 hover:bg-yellow-700 text-white font-bold h-12 px-6"> ZERAR PLACAR DAS TRIBOS </Button>
                        </div>
                    </div>
                )}
            </main>

import { getLoggedUser } from "@/lib/auth"; // <--- NOVIDADE: Importamos a função de segurança
import { prisma } from "@/lib/prisma";
import { Flame, Trophy, Calendar, User as UserIcon, Shield } from "lucide-react";
...
export default async function Home() {
  ...
  // 1. BUSCAR O USUÁRIO LOGADO (Usando nosso novo sistema de Auth)
  const dbUser = await getLoggedUser();

  // Se não tiver ninguém logado, mostra a tela de "Bem-vindo" com opção de entrar
  if (!dbUser) {
    return (
  ...
  // 2. BUSCAR O RANKING (TOP 3 GUERREIROS)
  // Isso continua igual, buscamos no banco geral
  ...
  // 3. ORGANIZAR OS DADOS PARA A TELA
  const user = {
    name: dbUser.name,
    level: dbUser.level,
    xp: dbUser.xp,
        ...
        <div className="bg-gradient-to-b from-violet-900 to-slate-950 p-6 rounded-b-3xl shadow-2xl shadow-violet-900/20">
          <div className="flex justify-between items-center mb-6">
            <div className="flex items-center gap-3">
              <div className="h-12 w-12 rounded-full bg-violet-500 flex items-center justify-center border-2 border-white">
            ...
            {/* Botão de Sair (Opcional, mas útil) */}
            <div className="flex gap-2">
              <div className="bg-slate-800/50 p-2 rounded-xl border border-slate-700">
                <span className="text-xs font-bold px-2 py-1 bg-violet-600 rounded-lg">Lvl {user.level}</span>
              </div>
            </div>
          </div>
            ...
            <Card className="bg-slate-900 border-slate-800 hover:bg-slate-800 transition-colors cursor-pointer h-full">
              <CardContent className="p-4 flex flex-col items-center gap-3 text-center justify-center h-full">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-400">Ganhar XP</p>
                </div>
              </CardContent>
            </Card>
          </Link>

...
export async function salvarResultado(lessonId: string, acertos: number, totalPerguntas: number) {
    ...
    // 1. Verifica se já jogou essa lição (para não ganhar XP infinito)
    // Como simplificação, verificamos se existe algum QuizResult dessa lição para esse aluno
    ...
    // 2. Registra que jogou (Cria um resultado genérico para marcar como feito)
    // Pegamos a primeira pergunta só para vincular no banco
    ...
    if (primeiraPergunta) {
        await prisma.quizResult.create({
            data: {
                ...
                isCorrect: acertos > 0, // Simplificação
            }
        });
    }

...
export function QuizGameClient({
    ...
    async function handleOptionClick(option: string) {
        if (isAnswered) return; // Não deixa clicar duas vezes
    ...
    async function finishGame() {
        ...
        // Chama o servidor para salvar os pontos
        if (!jaFez) {
            // Correção aqui: passamos score + 1 se a última foi correta?
            // Não, o score já atualizou no clique.
            // Espera, o score atualiza no state, mas pode ter delay.
            // Vamos usar o valor atual.
            await salvarResultado(lessonId, score + (selectedOption === currentQuestion.correctOption ? 0 : 0), questions.length);
        }
        setSaving(false);
    }
                    ...
                    <CardContent className="p-8 space-y-6">
                        ...
                        {jaFez ? (
                            <div className="bg-slate-800 p-3 rounded text-sm text-yellow-400">
                                Você já tinha jogado este quiz, então o XP não contou desta vez.
                            </div>
                        ) : (
    ...
    return (
        <div className="min-h-screen bg-slate-950 text-white p-6 pb-20 flex flex-col">
                    ...
                    {['A', 'B', 'C', 'D'].map((opt) => {
                        ...
                        if (isAnswered) {
                            if (opt === currentQuestion.correctOption) {
                                btnClass = "bg-green-600 border-green-600 text-white hover:bg-green-600"; // A Correta fica verde
                            } else if (opt === selectedOption && selectedOption !== currentQuestion.correctOption) {
                                btnClass = "bg-red-600 border-red-600 text-white hover:bg-red-600"; // Se errou, fica vermelha
                            } else {
                                btnClass = "bg-slate-900 border-slate-800 opacity-50"; // As outras apagam
                            }
                        }
                        ...
                        return (
                            <Button
                                ...
                                variant="outline"
                                className={`w-full text-lg ${btnClass} transition-all duration-300`}
                            >
                <span className="w-8 h-8 rounded-full bg-black/20 flex items-center justify-center mr-3 text-sm font-bold shrink-0">
                  {opt}
                </span>
                                <span className="whitespace-normal leading-tight">{text}</span>

                                {isAnswered && opt === currentQuestion.correctOption && (
                                    <CheckCircle className="ml-auto text-white" />
                                )}
                                {isAnswered && opt === selectedOption && opt !== currentQuestion.correctOption && (
                                    <XCircle className="ml-auto text-white" />
                                )}
                            </Button>
                        );
                    })}
            ...
            {/* BOTÃO PRÓXIMO (Só aparece depois de responder) */}
            {isAnswered && (
